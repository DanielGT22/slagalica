    ...
    const renderChessboard = () => {
      ...
      for (let i = 0; i < 11; i++) {
        ...
        for (let j = 0; j < 11; j++) {
          ...
          //Sets the Pieces to their respective Color Tiles
          // const renderPieces = (cellId) => {
          //   const pieceColor = Object.keys(colors).find(color => colors[color].includes(cellId));
          //   if (isPiece && isClickable) {
              
          //     //Sets the  className
          //     const pieceClassName = `piece-${pieceColor}-${pieces.indexOf(cellId) % 4 + 1}`;
          ...
          grid.push(
            <div key={cellId} id={cellId} className={`chess-cell rounded rounded-circle ${isPiece ? 'has-piece' : ''}`}>
              ...
              {/* {isPiece && ( <div className={`piece-${pieceColor}-${getColorNumber(pieceColor)}`}></div> )} */}
            </div>  );  } } return grid; };
